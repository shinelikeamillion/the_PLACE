...
.right-banner {vertical-align：middle;
...
-->
		...
		<div class="logo-search-bar">
			<div class="search">
				<div class="search-form">
					...
					<span class="mdi-action-find-replace"></span> <input type="hidden" />
				</div>
			</div>
		</div>
		<div class="user-bar">
			...
			<div class="bubble-btn">
				<a class="mdi-av-playlist-add" id="post-btn" href="" title="发表"></a>
			</div>
			<div class="bubble-btn">
	...
	<div class="post-bubble shadow-z-4 bubble">
		...
		<label for="postPic"><a class="mdi-image-color-lens"
			style="color: #FFA000" href="#"></a> <span>Photo</span></label> <label
			for="postVideo"><a class="mdi-av-videocam"
			style="color: #F44336" href="#"></a> <span>Video</span></label>
	</div>
	<!-- post窗口 -->
	<!--
	作者：940788048@qq.com
...
-->
	<div class="post-dialog hidden">
		...
		<form id="textForm" class="post-Form" action="./PostServlet"
			method="get">
			<input type="hidden" name="owner_id" id="owner_id"
				value="${ USERINFO.user_id }" /> <label
				style="padding: 5px; width: 100%;">${ USERINFO.user_name }</label> <input
				style="font-size: 2em;" id="post_title" placeholder="Title:"
				name="post_title" /> <input id="input-pics" type="file"
				id="post_pics" value="your pics" name="post_pics" /> <input
				id="input-url" placeholder="videoWebPath:" id="post_url"
				name="post_url" />
			<textarea rows="3" placeholder="Your text here" id="post_content"
				name="post_content"></textarea>
			<input placeholder="tags : #tag1 #tag2" id="post_tags"
				name="post_tags" />
			<div class="close-btn" style="float: left;">
				<a class="mdi-content-clear"
					style="font-size: 1.6em; color: #e74c3c;"></a> <input type="reset"
					class="back-btn" value="" />
			</div>
			<div class="submit-btn" style="float: right;">
				<a class="mdi-content-send"
					style="font-size: 1.6em; color: #0066FF;"></a> <input
					class="back-btn" value="" />
			</div>
		</form>
	</div>
		...
		<div class="discover-posts">
			<div class="posts-hoder">
				<div class="right-banner">
					...
					</a> <a class="mdi-social-people-outline" href="#"> Followers<span>${ USERINFO.user_follwedNum }</span>

					</a> <br> <a class="mdi-content-gesture" href="#"> Activity<span></span>
					...
					</a> <br> <a class="mdi-social-people" href="#"> Following <span>${ USERINFO.user_follwingNum }</span>

					</a> <br> <a class="mdi-social-person-add"
						href="Search.jsp?find=users"> Find Others <span></span>

					</a> <br>
					<hr>
					RADAR <br>
					<div class="mdi-social-whatshot">Gusst you like</div>
					<br>
					...
					<c:if test="${ friendsPosts.post_status == 0 }">
						<div class="user-post">
							...
							<div class="user-news">
								...
								<div class="post-title">Title：${ friendsPosts.post_title }
									{ ${ friendsPosts.post_tags }}</div>
								<textarea readonly>${ friendsPosts.post_content }</textarea>
								<div class="post-footer">
									<label class="notes">notes</label> <label class="repost"></label>
				...
				function() {
					...
					$(".mdi-action-subject")
							.click(
									function() {
										var userNmae = $(
												'.name-email span:first-child')
												.html();
										location.href = "Search.jsp?point="
												+ userNmae;
									});
					//				打开post窗口
					...
					$(".close-btn , .overlay").click(
							function closeDialog() {
								...
								$(".submit-btn").children("a").attr("class",
										"mdi-content-send");
								$(".submit-btn").children("a").css("font-size",
										"2em");
					...
					$(".submit-btn").click(
							function() {
								...
								$.get(url, function(data, status) {
									if (data == "true") {
										$(".submit-btn").children("a").attr(
												"class",
												"mdi-action-verified-user");
										$(".submit-btn").children("a").animate(
												{
													fontSize : "4em"
												}, "slow", function() {
													$(".close-btn").click();
